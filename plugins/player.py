...
async def generate_cover(requested_by, title, views, duration, thumbnail):
    ...
    draw.text(
        (190, 670),
        f"Powered By: EBD WORLD (@EBD_WORLDS)",
        (255, 255, 255),
        font=font,
    )
    img.save("final.png")
...
async def play(_, message: Message):
    ...
    if audio:
        ...
        title = file_name
        thumb_name = "https://te.legra.ph/file/dc26ba224897271279e81.jpg"
    ...
    if int(chat_id) in ACTV_CALLS:
        ...
        await message.reply_photo(
            photo="final.png",
            caption="**💥 𝔼𝔹𝔻𓂀𝕎𝕆ℝ𝕃𝔻🤞Ʌɗɗɘɗ 💿 Søɳʛ❗️\n🔊 Ʌʈ 💞 Ƥøsɩʈɩøɳ » `{}` 🌷 ...**".format(position),
...
async def pause(_, message: Message):
    ...
    await message.reply_photo(
                             photo="https://te.legra.ph/file/dc26ba224897271279e81.jpg", 
                             caption="**💥 𝔼𝔹𝔻𓂀𝕎𝕆ℝ𝕃𝔻 🔈 Mʋsɩƈ🤞Nøω 🥀\n▶️ Ƥɑʋsɘɗ 🌷 ...**"
    )
...
async def skip(_, message: Message):
    ...
    if int(chat_id) not in ACTV_CALLS:
        await message.reply_text("**💥 𝔼𝔹𝔻𓂀𝕎𝕆ℝ𝕃𝔻 💞 Ɲøʈɦɩɳʛ 🔇\n🚫 Ƥɭɑyɩɳʛ 🌷 ...**")
    else:
